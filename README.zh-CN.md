...
登录后可从左侧直接进入「AI 助手」，用自然语言搜索消息与工作数据、查询 ADO 工作项/PR/构建，
或生成工作项创建草案；所有创建操作仍需在现有创建窗口中确认。桌面端另有独立「Codex」入口，
可选择本地项目目录并新建或恢复隔离会话。DeepSeek 等 Provider 与密钥在「设置 → AI 管家」配置，
密钥只进入操作系统凭据库。
...
pnpm test:regression # 254 项回归：搜索并发、目录/成员分页、讨论访问与初始滚动、
                     # ADO 链路、任务栏与托盘、账号隔离、共享 Agent 与 LAN/outbox
pnpm test:classify  # 5 项，打真实 RC：单聊/多人直聊/群组分类、会话排序
pnpm agent:runner:test # 构建固定 Runner，验证敏感文件隔离与只读上下文附件
...
- **正式发版**：推送 `release/vX.Y.Z` 临时分支 → workflow 自动创建同名标签、删除临时分支，
  并构建三平台安装包与草稿 Release；标签只接受最新 `main`、一致版本、两位外部开发者证据和
  真实 README 截图/GIF；
- **公开发布**：受保护的 `Publish npm packages` 按 SDK → CLI 顺序发布，复核草稿后再手动运行
  `Publish GitHub Release`；创建标签不会自动公开 Release；
...
共享 Agent 还需要 Docker Desktop 与已登录的 Codex CLI。进入「设置 → AI → 共享 Agent Runner」
可检查依赖并一键安装固定的 `codex-cli 0.144.4` Runner；开发环境也可执行
`pnpm agent:runner:build`。Docker 不可用时不会降级到宿主原生进程。
