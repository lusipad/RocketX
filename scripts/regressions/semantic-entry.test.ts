...
test('embedding 语义搜索已整体移除，搜索入口改为创建当前 AI 运行时任务（issue #95）', () => {
  ...
  assert.doesNotMatch(readFileSync('apps/web/src/components/AiSettings.tsx', 'utf8'), /能力路由|embed/iu);
  ...
  assert.match(switcher, /handoffToButlerTask\(query, `搜索 · \$\{query\}`\)/);
  assert.match(switcher, /创建 AI 管家任务，并由当前运行时查找真实信息/);
