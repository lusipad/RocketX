...
test('默认逐条通知，开启禅模式后才聚合普通消息', () => {
  assert.deepEqual(routeNotification(normal, DEFAULT_NOTIFICATION_AGGREGATION_CONFIG), {
    mode: 'passthrough',
    reason: 'aggregation-disabled',
  });
  const zenMode = { ...DEFAULT_NOTIFICATION_AGGREGATION_CONFIG, enabled: true };
  assert.deepEqual(routeNotification(normal, zenMode), {
    mode: 'aggregate',
  });
  assert.deepEqual(
    routeNotification({ ...normal, directMention: true }, zenMode),
    { mode: 'passthrough', reason: 'direct-mention' },
  );
  assert.deepEqual(
    routeNotification({ ...normal, priority: 1 }, zenMode),
    { mode: 'passthrough', reason: 'priority-one' },
  );
  assert.deepEqual(
    routeNotification({ ...normal, text: 'P0 故障，请立即处理' }, zenMode),
    { mode: 'passthrough', reason: 'keyword' },
  );
  assert.deepEqual(
    routeNotification({ ...normal, broadcastMention: true }, zenMode),
    { mode: 'aggregate' },
  );
});
...
test('配置和统计按服务器/用户隔离持久化，损坏数据回退安全默认值', () => {
  ...
  const existingEnabled = parseNotificationAggregationState(JSON.stringify({
    ...state,
    config: { ...state.config, enabled: true },
  }));
  assert.equal(existingEnabled?.config.enabled, true);
  ...
  assert.equal(recovered?.config.enabled, false);
