...
test('仓库自带的示例配置永远能过解析器(样例防腐锁)', () => {
  ...
  // 配套的工作项模板样例同样要合形(WiTemplatesConfig 形状)
  const templates = JSON.parse(
    readFileSync('docs/examples/wi-templates.sample.json', 'utf8'),
  ) as { defaultProject?: string; templates: { name: string; items: unknown[] }[] };
  assert.ok(Array.isArray(templates.templates) && templates.templates.length > 0);
