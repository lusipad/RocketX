...
test('仓库全部公开版本面与 v0.24.0 对齐', async () => {
  await verifyVersions('0.24.0');
});

test('0.x 发布不冒充 1.0 成熟度门禁', () => {
  assert.equal(requiresMaturityEvidence('0.24.0'), false);
...
test('待发布版本可以从 CHANGELOG 提取用户向 Release notes', async () => {
  const notes = await releaseNotes('0.24.0');
  assert.match(notes, /^# RocketX v0\.24\.0/m);
