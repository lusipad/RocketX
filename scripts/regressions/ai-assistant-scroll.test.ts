...
test('对话类页面打开时停在最新内容，滚上去阅读时不被拽回底部（issue #90）', () => {
  ...
  // AI 页面：对话流 + 发送后强制回底
  const aiPage = readFileSync('apps/web/src/pages/AiAssistantPage.tsx', 'utf8');
  assert.match(aiPage, /useStickToBottom\(\[\s*lines,\s*activity,\s*butlerError,\s*routineDraft,\s*steps,\s*\]\)/);
  assert.match(aiPage, /<div ref=\{scrollRef\} onScroll=\{onScroll\}/);
  assert.match(aiPage, /stickToBottom\.current = true/);
