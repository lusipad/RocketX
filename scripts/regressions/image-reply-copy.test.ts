...
test('用图片/文件回复时引用跟随第一个上传发出，话题上传不消费引用（issue #91）', () => {
  ...
  assert.match(chat, /const quote = !tmid \? get\(\)\.replyTo : null;/);
  assert.match(chat, /quoteLinkPrefix\(quote, get\(\)\.subscriptions, await ensureSiteUrl\(\)\)/);
  assert.match(chat, /index === 0 && quoteMsg \? \{ msg: quoteMsg \}/);
  // 发送确认弹窗要让用户知道这是一条回复
