...
test('今日纳入直接提及、到期事项、未结束工作项、相关 PR 和需关注构建', () => {
  ...
  const input = {
    ...
    builds: [
      { id: 11, buildNumber: '20260717.1', definition: 'CI', project: 'P', status: 'completed', result: 'failed', requestedFor: 'A', queueTime: '', finishTime: '', webUrl: 'https://ado/build/11' },
      { id: 12, buildNumber: '20260717.2', definition: 'CI', project: 'P', status: 'completed', result: 'succeeded', requestedFor: 'A', queueTime: '', finishTime: '', webUrl: 'https://ado/build/12' },
    ],
    ipmsg: [
  ...
  assert.deepEqual(first.map((item) => item.kind), ['build', 'todo', 'pr', 'ipmsg', 'mention', 'event', 'pr', 'todo', 'workitem']);
  ...
  assert.deepEqual(todayCompletion(next), { done: 2, total: 9, rate: 2 / 9 });
});
