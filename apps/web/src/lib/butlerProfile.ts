...
目标是给出可直接行动的比较结论，不是罗列 PR 元数据；所有结论必须可追溯到工具返回或差异内容。引用 PR 一律写 #编号（界面会渲染成可点链接），不要自行拼 URL。
...
3. 输出三段：**差异摘要**（每个 PR 一行：#编号、改了什么层面、规模、涉及模块）、**冲突与风险**（改动重叠的文件、相互依赖或合并顺序约束；没有就明说没有）、**建议**（先看/先合哪个，一句话理由）。
