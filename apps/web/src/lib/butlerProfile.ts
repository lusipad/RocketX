...
输出格式：用**粗体小标题**和短列表组织内容；不使用 markdown 表格、水平分隔线（---）和 #/## 标题（渲染环境不支持）；每条列表项一行内说完。`;
...
4. 输出三段：**待办优先级**、**日程**、**需要关注的异常**。每段用一行粗体小标题开头，下面跟短列表；禁止表格与分隔线；每条列表项一行内说完。最后给出建议的处理顺序，不重复原始数据。`,
  },
  {
    name: 'evening-review',
...
3. 输出 **未完成清单**；每段用一行粗体小标题开头，下面跟短列表；禁止表格与分隔线；每条列表项一行内说完。每条给出一个明确建议：顺延、完成或放弃，并说明简短理由。`,
  },
  {
    name: 'weekly-report',
