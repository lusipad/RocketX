...
use crate::native::dsh_discovery as dsh_discovery_policy;
use crate::native::dsh_process::{
    DshAgentAttachmentMetadata, DshBridgeRelease, DshConnectionLease, DshExitEvent, DshOutputEvent,
    ManagedDshBridge, ResolvedDshRuntime,
};
pub use crate::native::dsh_process::{
    DshAgentAttachmentRuntimePath, DshBridgeInfo, DshBridgeState, DshRuntimeProbe,
};
use crate::native::dsh_runtime as dsh_runtime_policy;
use crate::native::process::{hidden_command, host_path, wait_for_exit};
...
pub(crate) use crate::native::dsh_discovery::{
    DSH_BRIDGE_ENTRY, DSH_BUNDLED_BRIDGE_ENTRY, DSH_BUNDLED_CLI_ENTRY, DSH_BUNDLED_RUNTIME_ARCHIVE,
    DSH_BUNDLED_RUNTIME_CACHE_DIR, DSH_BUNDLED_RUNTIME_DIR, DSH_BUNDLED_RUNTIME_SHA_MARKER,
    DSH_CONNECTIONS_SUBDIR, DSH_HOME_SUBDIR, DSH_NO_OPEN_VERSION, DSH_ROOT_DIR,
    DSH_SOURCE_CLI_ENTRY, DSH_VERIFIED_VERSION,
};

// Internal source-compatibility facade: tests and older in-crate callers can
// keep the dsh.rs names while discovery policy lives in native::dsh_discovery.

#[cfg(windows)]
#[allow(dead_code)]
fn find_program(name: &str) -> Option<PathBuf> {
    dsh_discovery_policy::find_program(name)
}
...
fn derive_source_root(candidate: &Path) -> PathBuf {
    dsh_discovery_policy::derive_source_root(candidate)
}

#[allow(dead_code)]
fn source_dsh_cli_entry(root: &Path) -> PathBuf {
    dsh_discovery_policy::source_dsh_cli_entry(root)
}

#[allow(dead_code)]
fn bundled_dsh_cli_entry(root: &Path) -> PathBuf {
    dsh_discovery_policy::bundled_dsh_cli_entry(root)
}

#[allow(dead_code)]
fn installed_dsh_cli_entry(node_modules_root: &Path) -> PathBuf {
    dsh_discovery_policy::installed_dsh_cli_entry(node_modules_root)
}

#[allow(dead_code)]
fn pnpm_global_dsh_cli_candidates(pnpm_home: &Path) -> Vec<PathBuf> {
    dsh_discovery_policy::pnpm_global_dsh_cli_candidates(pnpm_home)
}

#[cfg(windows)]
#[allow(dead_code)]
fn windows_dsh_shim_cli_candidates(shim: &Path) -> Vec<PathBuf> {
    dsh_discovery_policy::windows_dsh_shim_cli_candidates(shim)
}

#[allow(dead_code)]
fn source_bridge_path() -> PathBuf {
    dsh_discovery_policy::source_bridge_path()
}

#[allow(dead_code)]
fn packaged_bridge_candidates(app: &tauri::AppHandle) -> Vec<PathBuf> {
    dsh_discovery_policy::packaged_bridge_candidates(app)
}

#[allow(dead_code)]
fn resolve_packaged_bridge(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    dsh_discovery_policy::resolve_packaged_bridge(app)
}

#[allow(dead_code)]
fn bundled_bridge_path(root: &Path) -> PathBuf {
    dsh_discovery_policy::bundled_bridge_path(root)
}

#[allow(dead_code)]
fn development_bundled_runtime_root() -> PathBuf {
    dsh_discovery_policy::development_bundled_runtime_root()
}

#[allow(dead_code)]
fn development_bundled_runtime_archive() -> PathBuf {
    dsh_discovery_policy::development_bundled_runtime_archive()
}

#[allow(dead_code)]
fn bundled_runtime_cache_root(dsh_root: &Path) -> PathBuf {
    dsh_discovery_policy::bundled_runtime_cache_root(dsh_root)
}

#[allow(dead_code)]
fn bundled_runtime_sha_marker_path(root: &Path) -> PathBuf {
    dsh_discovery_policy::bundled_runtime_sha_marker_path(root)
}
...
fn prepare_bundled_runtime_root(
    ...
    let archive = dsh_discovery_policy::resolve_bundled_runtime_archive(
        &dsh_discovery_policy::bundled_runtime_archive_candidates(app),
    )?;
...
fn incompatible_node_message(version: &str) -> String {
    dsh_discovery_policy::incompatible_node_message(version)
}
...
fn bundled_node_paths(app: &tauri::AppHandle) -> Vec<PathBuf> {
    dsh_discovery_policy::bundled_node_paths(app)
}

#[allow(dead_code)]
fn probe_node_runtime(program: &Path) -> Result<(PathBuf, String), String> {
    dsh_discovery_policy::probe_node_runtime(program)
}
...
fn node_runtime_candidates(
    ...
    dsh_discovery_policy::node_runtime_candidates(system, bundled, use_private_node)
}
...
fn resolve_node_runtime(
    ...
    dsh_discovery_policy::resolve_node_runtime(app, use_private_node)
}

#[allow(dead_code)]
fn verify_installed_dsh_version(node_path: &Path, cli_path: &Path) -> Result<bool, String> {
    dsh_discovery_policy::verify_installed_dsh_version(node_path, cli_path)
}

#[allow(dead_code)]
fn dsh_version_is_compatible(version: &str) -> bool {
    dsh_discovery_policy::dsh_version_is_compatible(version)
}

#[allow(dead_code)]
fn dsh_version_is_unverified_newer(version: &str) -> bool {
    dsh_discovery_policy::dsh_version_is_unverified_newer(version)
}

#[allow(dead_code)]
fn dsh_version_supports_no_open(version: &str) -> bool {
    dsh_discovery_policy::dsh_version_supports_no_open(version)
}

#[allow(dead_code)]
fn dsh_root_paths(app: &tauri::AppHandle) -> Result<(PathBuf, PathBuf), String> {
    dsh_discovery_policy::dsh_root_paths(app)
}
...
fn resolve_dsh_runtime(
    ...
    dsh_discovery_policy::resolve_dsh_runtime(app, source_path, prepare_bundled_runtime_root)
}
