...
4. **房间浮层**：从房间右下角唤起、只显示当前房间相关事项和该房间管家会话。
...
- 折叠摘要只显示数量和最重要状态，例如“2 个会话”“3 项照看中”“1 项要确认”。
...
- 历史列表同时呈现普通管家会话、按房间隔离的管家会话，以及 AI 托管只读记录。
- 从一级“管家”进入时恢复普通管家会话；从房间浮层点“全屏”时才进入该房间会话，二者不能互相覆盖。
- AI 托管记录读取 Rocket.Chat 原文，不提供管家输入、重命名或删除；继续动作是回到原房间。
...
房间里的管家问答则使用按 `rid` 唯一的独立 Butler session；它与完整对话共享该房间
session，不再写进用户当前的普通管家 session。

## 表面四：管家管理

管家管理是 ButlerPage 内独立折叠区，不与今日正文混排。展开后分为四段行式列表：

1. **正在照看**：已装载的例行事务、开关、频率、上次运行；末尾进入能力库。
2. **记住的**：长期偏好及作用范围，可忘掉。
...
- **完整对话**：追溯用户和管家的原话。
- **AI 托管记录**：在同一历史栏只读查看房间中的 AI 托管交流，正文仍由 Rocket.Chat 持有。
...
- `useButler`：普通管家对话、房间管家对话与简报的单一来源；房间会话只保存 `rid` 和房间名作为来源身份。
- `sharedAgent` + Rocket.Chat 消息：AI 托管记录的单一来源；管家页只做只读投影，不把 transcript 复制进 Butler registry。
- `butlerErrandRuns`：派活、审批、TODO、停止、恢复和结论的单一来源。
- `useTodos`：用户待办的单一来源；纸面只做当天实时投影，不复制或重新持久化。
- 每日快照仓库：保存旧纸所需的只读投影；当天是实时数据，过去日期读快照。
- 房间浮层：事项按房间来源投影；问答读取该房间独立的 Butler session。
