...
| Suggestion | “要不要让 Butler 接着处理？” | 查看依据、转为任务、忽略/纠正、撤销 |
...
1. **持续**：关闭 Butler 页面后，已启用 Routine 与已确认 Task 仍继续；
...
Task 是“管家负责什么”的唯一合同。用户补充要求时生成新 `specVersion`，不覆盖历史。
...
| 任务 | Butler 当前负责什么？ | Task、Run、Artifact | 状态，再按回报时间 |
...
| 委托 | “把 PR #248 风险查清，三点前给我” | 生成 TaskSpec 预览并确认责任 |
...
任务视图不是项目管理看板，而是 Butler 已确认责任的可核验记录。
...
| Suggestion | 最多 3 行骨架 | 整个分区消失 | 不生成无依据建议 | 转为任务后迁移为 Task | 低置信度不展示，进入诊断记录 |
...
| Composer | 发送后锁定一次提交 | 保持自然提示语 | 恢复原稿并说明任务未创建 | 3 秒内回显合同 | 附件部分失败时允许移除后继续 |
...
目标不是让用户配置自动化，而是在 10 分钟内第一次感到“它看见了我真实的工作、开始推进了一件事，而且没有乱动手”。
...
- 用户至少确认过一条真实来源；
- Butler 创建一个 Task 或启用一个 Routine；
...
sequenceDiagram
    ...
    B-->>U: 正在处理：只读核对 PR、CI、回滚，15:00 前回来
    B->>E: 创建 Run #1
    E-->>T: 进度、来源、产物
    alt 只读即可完成
        E-->>T: verified result
...
| 第一次委托 | 不确定：“它真的开始了吗？” | 3 秒内给出目标、范围、回报时间 | 用户离开页面也放心 |
...
用户界面和模型生成示例中不使用“接住”“接成任务”“盯住”“盯它”“在盯”。这些词无法说明系统到底创建了任务、开始了执行，还是仅在周期检查。

### 9.2 好文案

```text
正在处理：只读核对 PR #248、相关 CI 和回滚步骤，15:00 前回来。
