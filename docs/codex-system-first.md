# 切片规格：Codex 系统优先，发布物不再捆绑（决策 15）

**决策 15**（2026-07-26，用户确认）：桌面端解析 Codex 的顺序翻转为**系统优先、捆绑兜底**；官方发布物**不再捆绑 codex 二进制**。决策 12 的「捆绑二进制」翻案。
...
- 不删捆绑扫描逻辑——已装旧版的用户靠它平滑过渡。
- 不做 codex 的按需下载——它有自己的安装与更新渠道，别替它管。
- #192（OCR 拆分）的账落地后重算，本刀不动 OCR。
