...
> **当前候选基线（2026-08-08，v0.36.5）：协议生成物 671 个一致、typecheck 与 Web 生产构建通过、test:pure 230/230、test:regression 970/970、test:ui 108/108、应用生态 clean-room 与插件发布包通过、Rust 桌面端 80/80、局域网插件 13 项通过且 2 项实网测试按设计忽略、pnpm 高危依赖审计无已知漏洞。Rocket.Chat 与 ADO 实网、本地多平台签名安装包仍由候选环境和受保护工作流复核。**——上两行的日期与数字是 v0.12.1 时点的历史快照，勿据此推导当前状态
