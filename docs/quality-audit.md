...
> **当前候选基线（2026-07-23，v0.29.1 待发布）：协议生成物 671 个一致、typecheck 通过、test:pure 221/221、test:regression 585/585、test:ui 41/41、Rocket.Chat 8.6.1 冒烟 53/53 且清理成功、会话分类 5/5、应用生态 clean-room 通过、插件发布包与 Web 生产构建通过、Rust 桌面端 43/43、内网通插件 13/13（2 项显式环境集成测试跳过）；v0.29.0 的 Windows/macOS universal/Linux 三平台 Desktop Build 已通过但保持未公开，v0.29.1 仍须走 Windows-only 受保护发布构建**——上两行的日期与数字是 v0.12.1 时点的历史快照，勿据此推导当前状态
