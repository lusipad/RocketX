...
> 日期:2026-07-14 · 基于 v0.6.0
> 来源:4 轮并行代码审查(状态/实时流、消息渲染/输入、本地持久化、功能模块)+ GitHub 12 个 Issue + 实机走查
> 基线:typecheck ✓、test:pure 157/157 ✓、smoke 46/46 ✓(打真实 RC)——但这些测不到渲染层,以下多数问题它们全绿也照样存在
...
- ✅ #18.7 通讯录只显示前 100 — searchUsers 加 offset,ContactsPage 翻页拉全(上限 5000)
...
**还没动(优先级更低的 P1/P2,见下方明细各条):**
- P1-3~P1-15 中未列入上面批次的、以及全部 P2(markdown 边界、输入法草稿、日历、搜索竞态等)
...
- **跨房间写错数据**:send/openThread/loadOlder/uploadFiles 都在函数开头捕获 rid 局部变量,按 key 写入,不会串房间(竞态只出在共享单例 timer,见 P2-k)。
