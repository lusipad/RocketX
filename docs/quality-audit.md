...
> **当前候选基线（2026-07-24，v0.31.1 待发布）：协议生成物 671 个一致、typecheck 通过、test:pure 220/220、test:regression 621/621、test:ui 50/50、Rocket.Chat 8.6.1 冒烟 53/53 且清理成功、会话分类 5/5、应用生态 clean-room 通过、插件发布包与 Web 生产构建通过、Rust 桌面端 51/51；v0.31.0 已按 Windows-only、非 Latest 策略公开，v0.31.1 仍须走同一受保护发布构建**——上两行的日期与数字是 v0.12.1 时点的历史快照，勿据此推导当前状态
