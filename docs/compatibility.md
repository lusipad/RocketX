...
Releases `v0.29.1` through the cancelled `v0.34.4` candidates were never promoted as a new cross-platform Latest. `v0.34.5` restored the official macOS and Linux packages, and `v0.40.2` continued that complete updater manifest. Starting with `v0.40.3`, the complete Windows, macOS, and Linux updater manifest continues to be promoted as GitHub Latest.
