...
Starting with `v0.29.1`, official desktop Releases are temporarily Windows x64 only. Windows users install the current `v0.30.0` release manually after it is public, while `v0.28.0` remains GitHub's Latest release and the last public cross-platform artifact set. macOS and Linux delivery, together with normal Latest-based auto-update promotion, resumes only after platform-specific validation is stable.
