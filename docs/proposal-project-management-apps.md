...
> 文档状态：**历史提案与一期实现记录**。工作台当前入口、视图、写操作确认和失败语义以[工作台规格](specs/workbench.md)为准。
...
> `ado.query` 能力总线。
>
> 看板拖拽改状态也已实现：确认弹窗后 json-patch 写入 `System.State`，
> 是否合法流转由 ADO 服务端裁决（`updateWorkItemStateRequest`）。
>
> 以下原方案保留作为二期参考：若未来要把看板/规划开放给第三方应用生态，
> 能力总线（§2）仍是正确的底座，第三方应用的写操作（`ado:write`）依赖它。
...
都跑在 M11 应用生态上（`@lusipad/rocketx`，iframe runtime，最小权限），不进主应用：
