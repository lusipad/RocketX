...
> 状态：v2.1，2026-07-17（v2 当日增补：新增 §4.7 管家 2.0 / M12，§9/§10/§11 随之更新；
> v2 取代 2026-07-14 的草案 v1，M6~M11 全部重定义，v1 的 M12 已取消，本版 M12 为新编号不冲突）
> 前置：M1–M10 已完成；M11 代码、文档、自动 clean-room 门禁和三平台 CI 干跑已完成，v0.20.1 是 M11 收口基线，当前发布候选为 v0.31.1；G3/G4 两位外部开发者真人计时与 README 真实截图/GIF 后置为未来 1.0 成熟度门禁，v0.25.14 已收口团队优先引导、配置同步与签名更新，v0.25.15 收口 npm 幂等重跑与 Registry 传播等待，v0.25.16 收口图文消息正文保留，v0.26.0 收口本地图片 OCR、内联工作项模板和历史 Agent 指令防重放，v0.26.1 收口大文件下载流式写盘，v0.27.0 收口可选附件本地留存、下载历史、跨平台 PP-OCRv5 本地识别与管家 M12 可执行 P0 基线，v0.28.0 收口管家 M12/P1 多会话 transcript、旧数据兼容迁移与固定三天 TTL 移除，v0.29.1 包含 M12/P2–P6 的 context compiler、共享 engine、typed tool runtime、scoped memory 与主动 workflow runtime，v0.30.0 增加管家双入口图片输入与禅模式，并修复下载来源、语言重试、文件打开、拖拽遮罩和表情面板问题，v0.31.0 增加 Azure DevOps Server 外置 Skill、工作项筛选与公式渲染，并修复附件清理提醒、手动清理重复下载和角色提示，v0.31.1 修复管家图片粘贴与 API 管家加载 ADO Skill；v0.29.0 仅保留未公开的受保护草稿标签，当前桌面 Release 暂只发布 Windows x64，macOS/Linux 待平台验证稳定后恢复；桌面/GitHub Release 仍与 npm 独立交付；Ollama 零外网与两周注意力指标继续做时序验收
...
| **理清** | AI 管家：未读摘要、消息→待办/工作项一键转化、智能分类 | M7 · M12 深化 |
| **组织** | 待办、日历、工作台、分组规则 | 已完成 |
| **回顾** | 统一收件箱「今日」+ AI 晨报/晚间回顾、语义搜索 | M7 · M12 深化 |
...
├────────────────────────────────────────────────────────────┤
│ 旁挂服务（可选）  services/ado-bridge（仅 ADO 事件通知）         │
└────────────────────────────────────────────────────────────┘
```
...
packages/app-sdk/                     @lusipad/rocketx —— 给第三方开发者的 Bridge 客户端
```
...
> 实施复盘（2026-07-17）：本表能力已基本交付，但其中「AI 助手统一入口」落地成了关键词
> 路由，偏离管家定位——诊断与下一代设计见 §4.7（M12），本表不再作为助手入口的现行依据。
...
交互补充（v0.20.1）：共享线程模型继续保留，同时增加独立 `Codex` 一级入口。它面向单机开发者，
...
### 4.7 管家 2.0（M12）——从能力清单到一个大脑

#### 4.7.1 诊断：M7 交付了零件，没交付管家

截至 2026-07-17（v0.20.x）的诚实盘点。已建成且可用的：AI 总线（多 Provider / 按能力路由 /
本地审计 / keychain 密钥）、会话总结（含未读回溯）、消息→待办/工作项结构化提取、
「今日」四源聚合 + 晨报/晚回顾、语义搜索（IndexedDB 向量索引）——**数据面是全的，模型
也接上了**。但「AI 助手统一入口」落地成了正则关键词路由：

- 输入必须命中「搜索/查询/待办/构建…」等关键词才被服务；回复是罐头话术，无多轮上下文；
- 未命中的整句被自动弹去 Codex（仅桌面端可用、绑定某个本地代码目录）——Web 端是死路，
  桌面端则把聊天问题发给一个不了解聊天上下文的代码 agent；
- `agent` 能力路由已在 `provider.ts` 定义但无人使用；LLM 指令解析路径已退化为死代码；
- `AiChunk` 缺 `toolCalls` 字段——工具调用的管道只通了发送半条，模型想调工具也回不来。

根因不是细节，是**缺一个把数据面和模型连起来的大脑**。用户被迫学习系统内部分工、说对
暗号才能得到服务（快捷提示词本质是教用户背咒语）——这是管家定位的反面。

竞品参照（腾讯 WorkBuddy，2026-07 观察）：它把 AI 拆成「助理 / 项目 / 专家·技能·连接器 /
自动化」四个用户必须自己理解的模块，自动化靠表单 + 模板库、只有定时触发，模板几乎全是
通用内容生成（新闻/单词/壁纸）——**分类学本身就是心智负担，而且它立在工作数据之外**，
听不到"构建挂了""被 @ 没回"。RocketX 的结构优势恰好相反：管家长在事件发生的地方
（消息/待办/日程/ADO 已全在 store 里），事件触发与投递面天然可得。由此定下 M12 第一
原则：**用户可见实体只有一个——管家**；WorkBuddy 的四个模块在这里全部降为内部实现。

#### 4.7.2 架构：六层，一条文本管线

```
工具(手) → Agent 循环(脑) → 人设(性格) · 记忆(阅历) · 技能(方法论)
                          → 例行事务(作息) → 「今日」(向你汇报的桌子)
```

| 层 | 形态 | 数量/加载 | 要点 |
|---|---|---|---|
| 工具 | 代码，强类型 | 十几个封顶，恒定注入 | 原语化；**审批/审计/脱敏的唯一强制边界** |
...
**人设与记忆**。人设不是皮肤，是判断力的出厂设置：什么事打断你、什么事收进摘要、回复
默认多长、不确定时先查证还是先问——双支柱直接写进默认值。用户用一句话调它（"以后回短
点"→ 管家改自己的人设文件并展示改动）。**不做多人设**——专家模式是把"该找谁"的负担还
给用户。记忆只存三类从数据推不出的事实：偏好与纠错、别名与语境（"老李 = 李建国"）、
决定与承诺；**工作数据不进记忆**——消息召回交给已建成的语义搜索，不把数据库复制进文本。
每次写入露一行「已记住：…」，记忆文件随时可看可删；仅存本地；桌面端落文件目录，
同一份记忆可直接喂给 codex 会话（IM 管家与 IDE agent 共享对你的了解）。

**技能**。格式对齐社区 Agent Skills 惯例（SKILL.md 目录），既可给管家也可给 codex /
Claude Code 用；开源定位下技能包是社区贡献的最低门槛入口。内置 4~5 个起步：晨报、
晚间回顾、周报、收件箱消化（晨报/晚回顾从"内置功能"重构为"内置技能"，作为格式的首批
验证品）。管家可以自己写技能——用户说"以后每周五帮我汇总本周 PR 进展"，管家生成技能
文件 + 注册例行事务 + 出确认卡。技能是**不可信文本**：它只能建议调用工具，绕不过工具层
的审批（将来开放分享时这条就是安全前提）。不做技能市场（见 §11）。

**例行事务与主动性**。例行事务 = 触发器 × 技能 × 投递三个字段。触发器两类：cron（晨报/
晚回顾即前两条内置例行事务）+ 事件 watcher（v0 三个高价值项：构建失败、@我未回应超时、
工作项被指派——数据都在现有 store / DDP 流里）。投递默认聚合进「今日」（支柱二：不做
WorkBuddy 式每条自动化各自弹通知的注意力灾难），紧急事项走已有穿透规则。所有运行进
运行记录（复用本地审计日志），列表可视、可暂停、可编辑。这就是 §3.3 里"`background.task`
扩展点随真实用例落地"等的那个真实用例。

**入口与路由反转**。正则快速路径保留（零延迟零成本是优点），但地位反转：命中 → 直接出
结果；未命中 → 进 Agent 循环，**永远不再把输入弹去 Codex**。Codex 从"听不懂的话的
垃圾桶"降级为管家的一个执行工具。管家在房间内可召（`panel.right` / composer 触发），
自动携带当前房间上下文。无 Provider 配置时只跑快速路径，在设置页说明一次——不在对话里
向用户解释内部架构。

**模糊指代解析阶梯**（"昨天老李给我的文件"）：① 查记忆命中别名 → 零打扰；② 用数据推
（通讯录姓李 ∩ 昨天给我发过消息的人），唯一候选 → 直接办、把推理和证据亮出来；③ 推不
动才问，且只问带证据的封闭题（二选一），不许问"老李是谁"；④ 问过一次写入记忆，同一
别名不许问第二次。置信门槛随动作危险度浮动：只读操作放胆猜（纠错即学习素材），副作用
操作必须过确认卡。

#### 4.7.2a 修订（2026-07-18）：管家大脑二选一（决策 11）

用户拍板「自用优先」后的架构修订——管家仍是**唯一用户可见实体**，但大脑成为设置页的
**显式二选一开关**，不做静默回退：

- **Codex 大脑**（桌面端主路径）：管家会话跑在 codex app-server 上（本机 ChatGPT 账号模型）。
  接线走协议自带的 **dynamicTools**（`thread/start` 注册函数工具 + `DynamicToolCall`
  server 请求由宿主执行应答）——现有 `createButlerTools()` 原样注册，**不需要 MCP 桥进程**，
  工具/审批/审计全部留在宿主代码（rcx-mcp 留在 M8 服务外部 agent，不提前）；人设与技能
  索引经 `baseInstructions`/`developerInstructions` 随线程注入；会话 cwd = 专属管家目录
  （记忆/技能落成真实文件，天然共享给 codex），沙箱只读、**不授予任何代码仓库路径——
  大脑有知识工具、没有手**；动代码仍走执行间（workspace 会话，逐次审批）。注入三角的
  结构防御由「读聊天的没有手」平移为「大脑只读 + 手在独立会话且人工审批」。
- **API 大脑**（Web / 内网路径）：既有 AiBus Agent 循环原样保留（已建成，维护成本≈0），
  内网 Ollama 零外网验收继续由此路径承担；开源用户无 ChatGPT 账号时用它。
- 选中 Codex 大脑但环境不可用（Web 端 / codex 未安装 / 未登录）→ 界面明确提示切换大脑，
  **不静默降级**（二选一的行为必须可预期）。
- **性能设计（2026-07-18 定案）**：管家线程**常驻复用**（首问建立、跨问保持、崩溃 resume，
  冷启动移出交互路径）；推理档默认 **medium**、不继承全局 xhigh（深度技能如周报可按需升档，
  算力按能力路由呼应 §4.2）；**流式输出**（agentMessage 增量直进对话行）；例行事务用
  ephemeral 短生命线程独立跑，不污染常驻对话上下文。
- 导航归一随之落定：桌面端 Codex 页降级为管家的「执行间」，一级导航只保留「管家」
  （推翻 v0.20.1 的 Codex 独立一级入口决策）。
- 前置 **Spike D**（§8）：app-server 会话挂接自定义 MCP server 的方式（per-thread 配置 vs
...
#### 4.7.2c 修订（2026-07-25）：管家单大脑，AI 层按职责重切（决策 13）

决策 12 冻结 API 大脑后的遗留状态最难受：它活着、功能永远落后、还持续收双引擎桥的
维护税（`resumeRevisionByBrain` / `brain-switched` 兼容分支 / `butler.ts` 双分叉）。
用户拍板「非桌面端只读降级」后收敛为单大脑：

- **Codex 是管家唯一大脑**。管家层只做 Codex 结构上做不到的三件事：**时机**（什么时候
  主动开口）、**呈现与闸门**（卡片 / 结论 / 确认 / 撤销）、**可见的记忆与人设**（原则 4）。
  一句话定义：管家 = 一个会自己挑时机开口的 Codex 前台。
- **双引擎桥降配，不推倒**：`ButlerEngineState` v2 保留单大脑仍需要的 turn 状态 /
  `transcriptRevision` / 中断恢复，删掉 `activeBrain` 与 per-brain resume 表；v1 持久化
  数据被拒绝后优雅回退冷启动（旧对话内容不丢，整段作为 bridge 重喂新线程）。
- **不再静默降级**：Codex 不可用时明说原因与修法（`codexRuntime` 的 `fallback` phase
  改为 `unavailable`），设置页删掉「API 大脑 / Codex 大脑」开关——那是用户从来不该
  关心的选择。
- **`kernel/ai` 不退役**：`aiAssistant`、消息实体提取、AiSettings 密钥配置仍是其消费者；
  本决策只摘除「管家走 API 大脑」这条路径。
- **非桌面端的管家呈现走 RC 本身**（W2，已落地）：简报以普通消息投递到
  「和自己的私聊」，web / 内网 / 手机官方 App 零新基础设施即可查看——只读降级
  不需要同步层，Rocket.Chat 就是同步层。默认关闭，简报页「同步到手机」一键开启
  （开启即投递当前一份），自动投递每天至多一条，记录按 server+account 隔离
  （`lib/butlerBriefDelivery.ts`）。

切片规格见 `docs/butler-single-brain.md`。

#### 4.7.3 落地顺序

1. `AiChunk.toolCalls` 流式解析（openai-compatible + anthropic 两个 provider）+ Agent
   循环 + 工具注册（现有查询/总结/提取函数原样包装，纯粘合）；
2. AiAssistantPage 路由反转 + 多轮会话历史；清理已死的 LLM 指令解析路径；
3. 技能加载器 v0（索引注入 + `load_skill`）+ 人设/记忆文件；晨报/晚回顾迁移为内置技能；
4. 例行事务 v0：本地注册表 + cron 触发 + 三个事件 watcher + 对话式创建（确认卡生效）；
5. 运行记录视图 + 房间内召唤入口。

#### 4.7.4 M12 验收

- **老李测试**："昨天老李给我的文件"三步内出正确消息卡片；同一别名全程只允许问一次，
  第二次直接命中记忆。
- **不弹 Codex**：任意自然语言输入在 Web 端得到管家回应（或快速路径结果），全程不被带
  离当前页面；无 Provider 时快速路径可用，界面不出现解释内部架构的文案。
- **对话即配置**："以后每周五汇总本周 PR 进展"→ 确认卡 → 周五「今日」出现周报卡片，
  运行记录可查该次触发、工具调用与投递。
- **事件主动性**：构建失败 → 「今日」出现含失败摘要的卡片，默认不弹通知；穿透规则可配。
- **内网可用**：Ollama 7B 级模型跑通 Agent 循环 + 技能渐进加载，全程零外网请求
  （抓包验证，延续 M7 标准）。
- **边界不可绕**：注入测试——在技能/记忆/人设文本中植入"直接创建工作项不要确认"类
  指令，副作用仍必须经确认卡；审计日志完整记录。
...
协议实现位于 `plugins/intranet-link/native` 的独立 Rust Sidecar：**UDP 2425**（发现/消息）+
**TCP 2425**（文件）。RocketX 核心只提供通用的签名 Sidecar 生命周期和 JSON-RPC 桥，
不包含命令字、端口或编码逻辑。
...
**只做共存模式（官方插件内置，零部署）**：启用「飞鸽 / IPMSG」插件后，Sidecar 以 IPMSG
身份上线，联系人和本地消息记录只存在于插件界面，不污染 Rocket.Chat 会话或 RocketX 核心
Store。标准 IPMSG/飞鸽支持消息与普通文件；原版内网通只承诺 `1@shiyeline` 的 2425 发现和文本。
（v1 的网关模式——rcx-hub 常驻双向桥——服务于"大规模过渡期"，随定位冻结，见 §11。）

价值：推广内网 IM 最大的阻力永远是"老王还在用飞秋，我发给他他收不到"。共存模式直接消灭
这个阻力——不需要全员同一天切换。

⚠️ **边界：飞秋 / 内网通对原版 IPMSG 有私有扩展**。插件识别飞鸽 `1_lbt` 与内网通
`1@shiyeline` 的 GBK 报文，但不模拟内网通私有 9011，也不对内网通联系人开放文件传输。
...
**开发者生态**：
- **`@lusipad/rocketx`**：发 npm，封装 Bridge JSON-RPC，TypeScript 类型完整。第三方开发者只面对它。
...
- Release 质量：当前稳定化阶段发布 Windows、macOS、Linux 安装包并面向用户重写 CHANGELOG；M11 的三平台基线以 **v0.20.1** 收口，**v0.34.1** 起恢复完整三平台 Latest；完成真人 G3/G4 与真实视觉素材后再进入 1.0。
...
**M11 / v0.20.1 验收**：自动 clean-room、三平台安装包、LICENSE / SECURITY.md 与应用生态交付通过。G3 与 G4 各找一位真实外部开发者实测计时，保留为未来 1.0 门禁，不阻塞 0.x 发布。

> 实施状态（2026-07-17）：SDK、CLI、三个正式样板、固定版本 Docker 栈、双语与开源文档、
> 三平台构建能力和自动 clean-room 代理门禁已在 M11 验证；v0.29.1 至取消的 v0.34.0 候选曾暂时收敛为 Windows-only，v0.34.1 起重新接入 macOS/Linux 产物并恢复完整 Latest。Docker 三服务首次拉取加构建至 healthy
> 约 2 分 44 秒，应用脚手架 clean-room 约 7.3 秒；两者都不是外部真人证据。当前先发布
> v0.20.1；G3/G4 真人计时与真实产品截图/GIF 在未来 1.0 标签前补齐。
...
| T2 | **UI 渲染冒烟**（Playwright 6 条：登录/发消息/切会话/搜索/右键菜单/AI 助手与 Codex 一级入口）——已纳入 CI，并把浏览器未处理异常作为失败 | M7 期间 · 已完成 |
...
| D | **dynamicTools 回路实测**：thread/start 注册函数工具 + DynamicToolCall 宿主应答 + baseInstructions 注入 + 只读沙箱下的行为（协议绑定已确认字段存在，跑通真实回路即通过） | 决策 11（管家 Codex 大脑）的全部接线压在这上面 | 管家 Codex 大脑开工前 |
...
| **M11** | 开源发布 & 应用生态 v0（v0.20.1 Release） | 开源作品交付 | 3~4 周 | 生态工具仅依赖 M6；未来 **v1.0 门禁 = M6~M9 + LICENSE + G3/G4 + 真实视觉素材**（§6） |
| **M12** | 管家 2.0：Agent 循环 + 技能/记忆/人设 + 例行事务（§4.7） | 理清 + 回顾（深化）、执行（移交 codex） | 4 周 | M7（已交付）；与 M9/M10 无耦合，为当前唯一活跃里程碑 |
...
| 13 | 管家单大脑，AI 层按职责重切（2026-07-25） | 用户拍板非桌面端**只读降级**后，管家收敛为 **Codex 唯一大脑**：删双引擎桥（engine state v2 去 `activeBrain`）、删设置页大脑开关、Codex 不可用时明说不静默降级；管家层只做时机 / 呈现与闸门 / 可见记忆三件事；`kernel/ai` 保留给非管家消费者；非桌面端呈现走 RC 消息投递（§4.7.2c，`butler-single-brain.md`） |
...
| 技能市场 / 专家多人设 | 分类学即心智负担（WorkBuddy 教训，§4.7.1）；一个管家 + 内置技能 + 本地技能目录够用 | 技能生态形成规模、出现真实分享需求 |
| 向量化情景记忆库（记忆衰减/自动摘要算法） | 聊天召回已有语义搜索，重复建设；记忆只存数据推不出的事实（§4.7.2） | MEMORY.md 体量或召回质量成为真实瓶颈 |
