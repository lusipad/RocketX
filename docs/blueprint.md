...
- **Codex 大脑**（桌面端主路径）：管家会话跑在 codex app-server 上（本机 ChatGPT 账号模型）。
  管家工具集经 **rcx-mcp**（§4.5 反向 MCP，自 M8 提前）以 MCP 工具形式提供给 codex；
  会话 cwd = 专属管家目录（人设/记忆/技能落成真实文件，天然共享给 codex），沙箱只读、
  **不授予任何代码仓库路径——大脑有知识工具、没有手**；动代码仍走执行间（workspace
  会话，逐次审批）。注入三角的结构防御由「读聊天的没有手」平移为「大脑只读 + 手在
  独立会话且人工审批」。
...
| D | **codex 会话挂接自定义 MCP server 实测**：per-thread 配置 vs 全局 config.toml vs profile；rcx-mcp 桥进程形态与宿主数据面连接方式 | 决策 11（管家 Codex 大脑）的全部接线压在这上面 | 管家 Codex 大脑开工前 |
