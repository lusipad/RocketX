...
> 前置：M1–M10 已完成；M11 代码、文档、自动 clean-room 门禁和三平台 CI 干跑已完成，v0.20.1 是 M11 收口基线，当前发布候选为 v0.31.0；G3/G4 两位外部开发者真人计时与 README 真实截图/GIF 后置为未来 1.0 成熟度门禁，v0.25.14 已收口团队优先引导、配置同步与签名更新，v0.25.15 收口 npm 幂等重跑与 Registry 传播等待，v0.25.16 收口图文消息正文保留，v0.26.0 收口本地图片 OCR、内联工作项模板和历史 Agent 指令防重放，v0.26.1 收口大文件下载流式写盘，v0.27.0 收口可选附件本地留存、下载历史、跨平台 PP-OCRv5 本地识别与管家 M12 可执行 P0 基线，v0.28.0 收口管家 M12/P1 多会话 transcript、旧数据兼容迁移与固定三天 TTL 移除，v0.29.1 包含 M12/P2–P6 的 context compiler、共享 engine、typed tool runtime、scoped memory 与主动 workflow runtime，v0.30.0 增加管家双入口图片输入与禅模式，并修复下载来源、语言重试、文件打开、拖拽遮罩和表情面板问题，v0.31.0 增加 Azure DevOps Server 外置 Skill、工作项筛选与公式渲染，并修复附件清理提醒、手动清理重复下载和角色提示；v0.29.0 仅保留未公开的受保护草稿标签，当前桌面 Release 暂只发布 Windows x64，macOS/Linux 待平台验证稳定后恢复；桌面/GitHub Release 仍与 npm 独立交付；Ollama 零外网与两周注意力指标继续做时序验收
...
- Release 质量：当前稳定化阶段发布 Windows x64 安装包并面向用户重写 CHANGELOG；M11 的三平台基线以 **v0.20.1** 收口，当前发布候选为 **v0.31.0**，macOS/Linux 待平台验证稳定后恢复；完成真人 G3/G4 与真实视觉素材后再进入 1.0。
