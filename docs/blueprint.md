...
> 前置：M1–M10 已完成；M11 代码、文档、自动 clean-room 门禁和三平台 CI 干跑已完成，v1.0.0 正式发布仍等待 G3/G4 两位外部开发者真人计时、README 真实截图/GIF、npm scope 权限与三平台标签产物核验；Ollama 零外网与两周注意力指标继续做时序验收
...
- Release 质量：三平台安装包、CHANGELOG 面向用户重写、**v1.0 正式版**。
...
**M11 验收**：G3 与 G4 各找一位真实的外部开发者实测计时通过；LICENSE / SECURITY.md 存在。

> 实施状态（2026-07-17）：SDK、CLI、三个正式样板、固定版本 Docker 栈、双语与开源文档、
> 三平台发布工作流和自动 clean-room 代理门禁均已落地。Docker 三服务首次拉取加构建至 healthy
> 约 2 分 44 秒，应用脚手架 clean-room 约 7.3 秒；两者都不是外部真人证据。正式 v1.0.0
> 标签前仍须完成 G3/G4 真人计时，并补入真实产品截图与 GIF 演示。
...
| **M11** | 开源发布 & 应用生态 v0（v1.0 Release） | 开源作品交付 | 3~4 周 | 生态工具仅依赖 M6；**v1.0 门禁 = M6~M9 + LICENSE**（§6） |
