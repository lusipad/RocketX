...
> 前置：M1–M10 已完成；M11 代码、文档、自动 clean-room 门禁和三平台 CI 干跑已完成，v0.20.1 是 M11 收口基线，当前发布候选为 v0.29.0；G3/G4 两位外部开发者真人计时与 README 真实截图/GIF 后置为未来 1.0 成熟度门禁，v0.25.14 已收口团队优先引导、配置同步与签名更新，v0.25.15 收口 npm 幂等重跑与 Registry 传播等待，v0.25.16 收口图文消息正文保留，v0.26.0 收口本地图片 OCR、内联工作项模板和历史 Agent 指令防重放，v0.26.1 收口大文件下载流式写盘，v0.27.0 收口可选附件本地留存、下载历史、跨平台 PP-OCRv5 本地识别与管家 M12 可执行 P0 基线，v0.28.0 收口管家 M12/P1 多会话 transcript、旧数据兼容迁移与固定三天 TTL 移除，v0.29.0 收口 M12/P2–P6 的 context compiler、共享 engine、typed tool runtime、scoped memory 与主动 workflow runtime；#168、#173–#177 待公开 Release 验证后统一关闭；桌面/GitHub Release 仍与 npm 独立交付；Ollama 零外网与两周注意力指标继续做时序验收
...
- Release 质量：三平台安装包、CHANGELOG 面向用户重写；M11 以 **v0.20.1** 收口，当前发布候选为 **v0.29.0**；完成真人 G3/G4 与真实视觉素材后再进入 1.0。
