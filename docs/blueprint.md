...
> 文档状态：**愿景与历史路线图**。本文不承诺当前已实现；当前能力以[功能规格](specs/README.md)为准，GTD、注意力保护与 AI 边界以[产品原则](specs/product-principles.md)为准。下文版本、里程碑和“当前”措辞均是当时记录。
...
> 前置：M1–M11 已完成，v0.20.1 是 M11 收口基线；当前发布候选为 `v0.41.0`。
> v0.34.2 恢复三平台正式交付，v0.40.0 继续交付 Windows x64、macOS universal 与 Linux x64；
> 桌面/GitHub Release 仍与 npm 独立，G3/G4 外部开发者计时与 README 真实截图/GIF 后置为未来
> 1.0 成熟度门禁。各候选版本的完整收口记录以 [`CHANGELOG.md`](../CHANGELOG.md) 为准。
...
的审批（开放分享时这条就是安全前提）。RocketX 不另造专有技能市场；桌面端直接复用 Codex
原生 Plugin/Marketplace，旧 Markdown 粘贴入口只保留为兼容导入（见 §11）。
...
> 以下是 M12 v0 的历史落地顺序。当前 Codex 线程使用原生 Agent Skills 与 Plugin/Marketplace；
> `load_skill` 只出现在旧方案和 API 兼容记录中，不再作为 Codex Host Tool 暴露。
...
- **传输**：TCP 直连，1 MB 分片、并发流、断点续传（offset）、BLAKE3 校验、可选 TLS
  （自签 + RC 身份绑定）。
- **自动路由**：发文件时内核判断——对方在线 + 同网段 + 文件达到可配置阈值（默认 50 MiB）→ P2P；
  否则 → 走 RC 上传。首次直传成功给一次性说明，之后静默。
...
3. **共享盘路径解析**：粘贴 `\\nas\share\x.pdf` 直接预览。→ 已实现最小版（v0.43+）：消息里的
   UNC 路径渲染成可点击卡片，点击弹确认框（提示将连接的局域网主机）后用系统默认程序打开；
   仅桌面端，Web 端提示不可用。直接读内容做内嵌预览留作后续。
...
- Release 质量：当前稳定化阶段发布 Windows、macOS、Linux 安装包并面向用户重写 CHANGELOG；M11 的三平台基线以 **v0.20.1** 收口，**v0.34.2** 起恢复完整三平台 Latest；完成真人 G3/G4 与真实视觉素材后再进入 1.0。
...
> 三平台构建能力和自动 clean-room 代理门禁已在 M11 验证；v0.29.1 至未公开的 v0.34.1 候选未形成新的三平台 Latest，v0.34.2 起重新接入 macOS/Linux 产物并恢复完整 Latest。Docker 三服务首次拉取加构建至 healthy
...
| RocketX 专有技能市场 / 专家多人设 | 不重建 Codex 已有 Plugin/Marketplace；专家分类仍会增加心智负担（WorkBuddy 教训，§4.7.1） | Codex 原生市场无法满足真实分发需求，或多人设出现不可替代的真实场景 |
