...
- Release 质量：当前稳定化阶段发布 Windows x64 安装包并面向用户重写 CHANGELOG；M11 的三平台基线以 **v0.20.1** 收口，当前发布候选为 **v0.31.1**，macOS/Linux 待平台验证稳定后恢复；完成真人 G3/G4 与真实视觉素材后再进入 1.0。
...
> 三平台构建能力和自动 clean-room 代理门禁已在 M11 验证；当前官方桌面发布工作流暂收敛为 Windows-only，macOS/Linux 产物待稳定性证据充分后重新接入。Docker 三服务首次拉取加构建至 healthy
