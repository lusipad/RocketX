...
> 前置：M1–M10 已完成；M11 代码、文档、自动 clean-room 门禁和三平台 CI 干跑已完成，v0.20.1 是 M11 收口基线，当前发布候选为 v0.25.14；G3/G4 两位外部开发者真人计时与 README 真实截图/GIF 后置为未来 1.0 成熟度门禁，v0.25.14 收口团队优先引导、配置同步与签名更新；桌面/GitHub Release 仍与 npm 独立交付；Ollama 零外网与两周注意力指标继续做时序验收
...
- Release 质量：三平台安装包、CHANGELOG 面向用户重写；M11 以 **v0.20.1** 收口，当前发布候选为 **v0.25.14**；完成真人 G3/G4 与真实视觉素材后再进入 1.0。
