...
| 1.5 | **管家页主体翻转**：「今日」升为主体（状态头 / 今日简报 / 派出去的活 / 在盯的事入口 / 底部输入框），对话降为一层——窄窗从底部滑上，宽窗（≥1100px）固定右栏与今日并排。三表面 = 同一张图的三种裁切：桌面页完整、房间侧栏砍简报、展开层纯对话。组件全保留（刀 1 的活区即插即用），重写的只有 ButlerPage/ButlerConversation 的布局壳。空态即引导：无实体时今日区显示场景例句、对话自动展开。**一屏密度预算**（1080p 可视区装得下是硬约束）：简报最多 5 条、正在干最多 3 张单行、回话了最多 2 张，超出折叠成「还有 N 件 ›」；**等你点头的卡永不折叠**——全页唯一阻塞态，宁可挤折叠别的区也不许藏它。「收下」即清场，堆积只发生在「你一直不理」的场景，而那正该被折叠行提醒着 | 刀 1 |
