...
| 4 | **管家能力库 v1**（routines / 技能安装 / 感知规则三线合一）：一切主动感知收敛为「提示词 + 调度」的例行事务，与晨报/晚间回顾同一机制（routines.ts 骨架已有）。**能力库**是一批内置模板——@我必析、新 issue 必析、群讨论汇总、晨报、晚间回顾——用户**可选式装载**：装载即实例化为自己的例行事务（可改调度与提示词、可卸载），不装载的能力零成本（不轮询不执行）。模板即文本，与 SKILL.md 分发形态同源，开源后社区可分享。要补的差距：①调度粒度加 interval（每 N 分钟，现状仅 daily）；②库与装载 UI（挂「管家学到的」旁）；③**成本守卫**——高频任务先本地预检（有新 @ 才喂 codex，无新事速退不烧 token）。三条守卫不变：分析 read-only、建议动作转规格卡走审批、出口只有简报不新增弹窗 | 刀 1（建议动作转规格卡）、刀 1.5（「正在照看」入口挂新骨架） |
...
- issue 那条 v1 不重建轮子：管家工具面触不到 GitHub，而 codex automation「每小时处理新 Issue」已在跑，这条模板 = 把它的结论汇入简报。
- UI 挂管家页「正在照看」面板：上半已装载列表（开关 / 频率 / 上次运行一行小字），下半能力库（说明 + 装载按钮），底部「自己写一条——一段提示词加一个时间就行」。
