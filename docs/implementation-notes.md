...
- 去掉容器后，文件访问边界由 Codex 原生 OS sandbox 和用户选择的工作目录决定；RocketX 不再提供额外的容器级读取隔离。
...
- `scripts/regressions/codex-host-runtime.test.ts` 锁定本机 app-server、真实工作目录、原生 sandbox 参数以及不再构建 Agent Runner 镜像的架构合同。
...
- 独立 Codex 与共享 Agent 依赖本机 PATH 中兼容的 Codex CLI；Docker 仅用于可选的 Rocket.Chat 自部署栈。
...
- 明确的搜索和查询先由本地规则解析；模糊表达使用 `codex exec --output-schema` 输出白名单意图 JSON，
  RocketX 执行查询。DeepSeek 不再是 AI 助手入口的依赖。
...
- Codex 独立入口与共享 Agent 都复用本机锁定版本的 app-server，直接使用用户选择的真实本地目录，
...
- 当前 UI 自动化入口仍返回空能力，无法代替用户点击真实桌面窗口；已用导航注册契约、TypeScript、
  生产构建、256 项全量回归、219 项纯函数、Rust 与本机 app-server 架构回归覆盖可自动验证部分。

## Questions for review

- 真实 DeepSeek 密钥已在聊天中暴露，不再写入命令或日志；若继续使用 DeepSeek 可选能力，必须先轮换，
  但它不再阻塞 Codex、搜索、查询或工作项草案功能。

---

# Implementation notes — GitHub Issue #105 Codex App 交接

## Demo

Windows 实机通过 `codex://threads/new` 在 `rocketchatx / 本地 / main` 打开新任务，首轮返回
`ROCKETX_HANDOFF_OK`，同一任务第二轮返回 `SECOND_TURN_OK`。旧 app-server 验证线程只能进入侧栏，
继续后显示红色错误并进入 `systemError`。

## What and why

旧实现用 RocketX 启动的独立 Codex app-server 创建、命名并执行线程。生成的会话能够被 Codex App
索引，却不受 Codex Desktop 当前任务宿主托管，因此“侧栏可见”不等于“可以继续”。新实现使用官方
新任务 deep link，由 Codex App 自己创建并拥有可执行线程。

## Decisions

- 使用 `codex://threads/new?prompt=...&path=...` 预填完整记录和绝对工作区；不再伪造原生线程。
- 保留 Codex App 的发送确认边界，不尝试模拟自动发送。
- 原生 URL 校验器只放行 `threads/new` 的非空 `prompt` 和绝对 `path`，拒绝未知参数、相对路径与控制字符。
- URL 超过 8192 字符时完整复制记录并打开对应工作区，不做静默截断。

## Unknowns answered

- 本机 PATH Codex CLI 与 Desktop 内置运行时版本不同，但使用内置运行时重做旧路径仍然失败，排除单纯版本差异。
- 官方文档明确 `prompt` 只填入输入框，不自动发送；界面提示和交互据此改为“确认后发送”。
- 短记录已完成真实两轮对话验证；超长记录兜底由回归测试和剪贴板结果分支覆盖。

## Non-goals and limits

- 转交是快照，不与 RocketX 原会话双向同步。
- 当前未自动归档旧的失败验证线程。
- 超长记录仍需要用户粘贴并发送。

## How to try

1. 在管家或 AI 托管面板点击“转到 Codex App”。
2. 确认 Codex App 显示正确工作区与完整记录。
3. 发送预填内容并继续对话。

自动验证：`pnpm typecheck`、`pnpm test:regression`、`cargo test --locked`。
