...
| `pnpm test:regression` | 254 项回归测试：快捷搜索并发、本机即时结果、受控搜索范围、远端渐进补全与滚动分页，目录与成员分页、跨房间发送与结果隔离，ADO 配置/查询/链接卡片与工作项父子层级，讨论访问与订阅边界、初始滚动，任务栏与托盘未读状态、缓存边界、账号隔离、AI 助手、Codex 本地入口、共享 Agent 会话与发布合同 |
