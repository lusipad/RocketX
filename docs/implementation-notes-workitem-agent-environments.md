...
- 托管成功只发布一条 Rocket.Chat 标准 Markdown 状态消息，固定展示主持人、执行引擎、状态、环境、当前任务、ISO 租约时间和 `@ai` 用法；RocketX 使用该消息自身的 `_id` 作为跨设备租约 claim，不再把机器 JSON 混入正文，原版客户端只显示可读内容。任何能在当前房间正常发言的成员都可开启托管，客户端不再查询角色或成员目录；可读正文与历史 HTML / 隐形标记本身仍不是独立授权凭据，只有消息作者与卡片宿主一致、房间/话题匹配且租约不超过服务端消息时间窗口的卡片，才会进入跨设备宿主仲裁。用户还可在托管面板设置“进入本房间时自动开启”，该设置仅在本机生效且不会抢占其他宿主。
