...
把管家做成 RocketX 内的本地 AI 任务面：用户在设置里选择 Codex、DeepSeek 或无 AI，保存后重启生效，再使用当前启动级运行时的原生会话与配置能力。Codex 继续走 `app-server`；DeepSeek 由官方 DSH Web 提供，并在 RocketX 管家里以 iframe 宿主。RocketX 保留消息、工作台和个人效率等确定性界面，不自建 Agent 内核，也不增加抹平后端差异的通用 Runtime 层。
...
- 切换到独立 DeepSeek 视图，启动官方 DSH Web 并以 iframe 宿主其本地 loopback 页面；
- 使用官方 DSH Web 完成模型、提供方、推理强度、Agent preset、permission preset、approval、question 和 credentials 配置；
...
- 在 RocketX 中自建 DSH 会话/设置页，或复制一套与官方 DSH Web 并行的管理台；
...
- DeepSeek 视图使用当前托管项目目录；默认 slim 连接系统里已安装且可运行、且已被 RocketX 验证为 `0.1.0-rc.6` 的 DSH，Windows full 则使用同版本私有固定 DSH 运行时。RocketX 只负责通过 bridge 拉起官方 DSH Web、拿到本地 loopback URL 并嵌入 iframe，发送前需在官方 DSH Web 中配置 DeepSeek API Key；官方 DSH Web 没有外部输入注入协议，所以无房间目标的交接会复制提示词并打开 DSH Web，用户再粘贴发送。
- 性能模式关闭时才提供 AI 任务执行。

## 4. 主流程

1. 用户进入管家、选择托管项目，并使用当前启动级 AI 运行时。
2. Codex 视图探测 Runtime、启动 `app-server`，加载模型、权限、Skills、Apps、Plugins 和 Thread 列表。
3. DeepSeek 视图由 Tauri 启动随包 DSH bridge，bridge 拉起官方 DSH Web 并返回严格的 `127.0.0.1` loopback URL；RocketX 只负责 iframe 宿主和进程生命周期，模型、Agent preset、permission preset 与 credentials 都在官方 DSH Web 内配置。
...
DeepSeek 视图不复制上述 Codex 专属动作。模型、Agent preset、permission preset 和 credentials 都由官方 DSH Web 配置；RocketX 只保留进程宿主、严格 loopback URL 校验和 sandbox iframe。AI 托管与共享 Agent 仍通过独立的 controller/host 消息路径和 DSH 原生 RPC 交互，不走这张嵌入页。无房间目标的交接会把提示词复制到剪贴板并打开官方 DSH Web，用户需要手动粘贴发送。
...
| 桌面端 + 可用 DSH | 已实现 | 官方 DSH Web iframe 宿主，外加原生 Session、配置、审批、提问与凭据流程 |
...
- DSH Session、默认模型/Agent/权限和凭据由官方 DSH Web / DSH 管理；API Key 由 DSH `credentials-local` 写入 `.credentials.yaml`，RocketX 外壳不读取、不复制也不回显密钥或配置正文。
...
- DeepSeek iframe 只接受 bridge 校验后的 `127.0.0.1` URL，并受 sandbox 能力白名单约束；bridge 在返回 URL 前必须完成 DSH `host.describe` 探测。
...
| DeepSeek API Key 未配置 | 发送前提示配置 | 不发送 prompt；在官方 DSH Web 配置后重试 |
...
- `BUT-AC-08`：Codex 与 DeepSeek 受同一启动级 AI 运行时选择约束；DeepSeek 视图嵌入官方 DSH Web，只展示该后端真实提供的配置和动作。
- `BUT-AC-09`：DSH 模型/提供方、推理强度、Agent preset、permission preset、approval、question 与 credentials 都由官方 DSH Web / 原生 RPC 驱动，而非 RocketX 自绘配置页。
...
- 实现：`apps/web/src/pages/ButlerPage.tsx`、`apps/web/src/components/DshConversation.tsx`、`apps/web/src/components/DshQuestionCard.tsx`
- 实现：`apps/web/src/agent/dsh/DshController.ts`、`apps/web/src/agent/dsh/HostedDshController.ts`
...
- 自动化：`scripts/regressions/dsh-controller-runtime.test.ts`、`scripts/regressions/hosted-dsh-controller.test.ts`、`scripts/regressions/dsh-bridge-script.test.ts`、`scripts/regressions/dsh-view.test.ts`
...
- DeepSeek 视图开放的是官方 DSH Web 已有模型/Provider、Agent 与权限的选择，不包含 RocketX 自绘的 provider endpoint CRUD 或另一套 Agent preset 编辑器。
