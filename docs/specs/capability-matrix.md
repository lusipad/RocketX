# RocketX 能力矩阵

> 基线：工作树 `2026-08-19`，应用版本 `0.43.6`
> 判断口径：只记录当前可观察行为；未来目标见各能力域的“已知差距与目标”。

## 1. 场景定义

| 场景 | 定义 |
| --- | --- |
| 桌面端 + 兼容 Codex | Tauri 桌面端可解析已登录的 Codex，`app-server` 探测和版本门禁通过 |
| 桌面端，无可用 Codex | 未安装、未登录、版本被阻止，或 `app-server` 探测失败 |
| 桌面端 + 可用 DSH | slim 可连接已安装且可运行、且已被 RocketX 验证为 `0.1.0-rc.6` 的系统 DSH；Windows full 则使用同样锁定为 `0.1.0-rc.6` 的私有固定 DSH 运行时；发送时已配置当前所选 provider 需要的凭据 |
| 网页版 | 浏览器访问 RocketX，不具备 Tauri 本地进程与文件能力 |
| 性能模式 | 用户主动关闭 Codex、DSH、例行任务、共享 Agent、本地 OCR 等高开销能力 |

矩阵中的“受限”表示功能存在，但还依赖服务器设置、权限、平台或本机进程；不表示功能缺失。

桌面端的 AI 能力共享同一个启动级选择：Codex、DSH 或无 AI。设置保存后要重启 RocketX 才会生效，当前进程不会热切换，也不会同时启动两个后端。下表里所有 AI 行都按这一前提解释。

## 2. 当前能力

| 能力 | 桌面端 + 兼容 Codex | 桌面端，无可用 Codex | 网页版 | 性能模式 | 说明 |
| --- | --- | --- | --- | --- | --- |
| 首次引导与团队配置 | 已实现 | 已实现 | 部分实现 | 已实现 | 首次理念引导只拦截桌面新安装；设置页导入两端可见 |
| Rocket.Chat 登录与账号恢复 | 已实现 | 已实现 | 已实现 | 已实现 | 网页版可留空服务器地址并使用当前站点 |
| 消息、线程、反应与会话列表 | 已实现 | 已实现 | 已实现 | 已实现 | 依赖 Rocket.Chat REST 与实时连接 |
| 中文用户搜索 | 已实现 | 已实现 | 已实现 | 已实现 | 结果仍受 Rocket.Chat 用户目录权限和服务端返回限制 |
| 中文消息子串搜索 | 受限 | 受限 | 受限 | 受限 | 服务端需允许正则搜索，例如 `Message_AlwaysSearchRegExp=true` |
| 文件上传与浏览器下载 | 已实现 | 已实现 | 已实现 | 已实现 | 网页版使用浏览器文件能力 |
| 原生保存、打开、定位与下载记录 | 已实现 | 已实现 | 不适用 | 已实现 | 仅桌面端具有本地路径和系统文件管理器能力 |
| 工作台与 ADO 确定性查询 | 受限 | 受限 | 受限 | 受限 | 依赖 ADO 连接、凭据和服务器网络；浏览器不能使用 NTLM |
| ADO 我的代码 | 受限 | 受限 | 受限 | 受限 | 默认汇总最近一年当前身份创建的 PR；旧 Server、权限和项目局部失败会明确标注部分覆盖 |
| ADO 写操作 | 受限 | 受限 | 受限 | 受限 | 只通过显式动作执行；结果未知时不得自动重试 |
| 待办、日历、通讯录 | 已实现 | 已实现 | 已实现 | 已实现 | 待办和日历以本机数据为主；通讯录来自 Rocket.Chat |
| 启动级 AI 运行时选择 | 已实现 | 已实现 | 已实现 | 不适用 | 在设置里选择 Codex / DSH / 无 AI；保存后重启生效，不热切换，不同时启动两个后端；performance 模式关闭 AI 并隐藏这一入口 |
| 管家 Codex 原生任务 | 已实现 | 不可用 | 不可用 | 不适用 | 仅在启动级选择为 Codex 时可用；桌面端本地 `app-server` 传输 |
| 管家 DSH 原生任务 | 受限 | 受限 | 不可用 | 不适用 | 仅在启动级选择为 DSH 时可用；slim 依赖已安装且已被 RocketX 验证为 `0.1.0-rc.6` 的系统 DSH，Windows full 依赖同版本私有 DSH 运行时；DSH 视图会启动官方 DSH Web 并把本地 loopback URL 以 iframe 嵌入，AI 托管和共享 Agent 仍走 controller/host 路径，并沿用 DSH 当前 provider 及其凭据 |
| DSH 模型 / Provider / 推理 / Agent / 权限配置 | 受限 | 受限 | 不可用 | 不适用 | 私人会话由官方 DSH Web 配置；共享托管开启页从 DSH 原生目录读取本次模型、推理、Agent 与权限，不复制持久配置或自绘管理台 |
| 管家与 Codex App 接续 | 受限 | 不可用 | 不可用 | 不适用 | 仅在启动级选择为 Codex 时可用；支持打开和手动刷新；同一线程顺序接续，不同任务线程可在共享 Runtime 中并行 |
| Skills / Plugins / Apps 目录 | 已实现 | 不可用 | 不可用 | 不适用 | 目录和动作来自当前 Codex `app-server` |
| Codex 原生 Memory | 受限 | 不可用 | 不可用 | 不适用 | 每个管家线程启用；还受 Codex 全局 Memory 配置影响 |
| 已安排任务的配置与立即运行入口 | 已实现 | 部分实现 | 部分实现 | 不适用 | 仅在启动级选择为 Codex 时可执行；无 AI / 网页版能配置但不能完成 AI 执行，应明确失败边界 |
| 已安排任务自动触发 | 受限 | 不可用 | 不可用 | 不适用 | 仅 RocketX 正在运行且启动级选择为 Codex 时由本机计时器触发，不是后台云任务 |
| 房间/话题共享 Agent（Codex） | 已实现 | 受限 | 受限 | 不适用 | 只有选择 Codex 且具备本地工作区/环境的桌面端能承载宿主；无本地执行器和 Web 客户端仍可查看有效远端租约并向该宿主使用 `@ai`，不能本地开启或恢复 |
| 房间/话题共享 Agent（DeepSeek） | 受限 | 受限 | 受限 | 不适用 | 只有选择 DSH 且运行时可用的桌面端能承载并恢复原生 DSH Session；无本地执行器和 Web 客户端仍可查看有效远端租约并向该宿主使用 `@ai` |
| 独立“派出去”委托 | 未实现 | 未实现 | 未实现 | 未实现 | 当前没有独立委托主流程，不得把历史界面当现状 |
| 任务内审批与结构化输入 | 已实现 | 受限 | 不可用 | 不适用 | Codex 请求回所属 Thread；DSH approval/question 回所属 Session；已安排任务仍只走 Codex 且不等待人工审批 |
| 桌面通知、托盘、开机启动、更新 | 已实现 | 已实现 | 不适用 | 已实现 | 正式安装版自启静默驻留；共享目录可签名，或在未签名时固定本次探测的 SHA-256 |
| 本地图片 OCR | 受限 | 受限 | 不适用 | 不适用 | 全量包才包含 OCR 资源；性能模式关闭该能力 |

## 3. Codex Runtime 兼容结论

| 条件 | 当前表现 |
| --- | --- |
| 未安装 Codex CLI/App | Rocket.Chat 与确定性工作台继续可用；管家本地执行、目录、Memory、已安排执行和本机共享 Agent 宿主不可用；有效远端托管仍可查看并使用 `@ai` |
| 安装版本低于协议基线 `0.144.4` | 被版本门禁阻止，不尝试启动任务 |
| 版本等于已验证基线 `0.144.4` | 标记为 `verified`，完成 `app-server --help` 与登录探测后可用 |
| 版本高于基线 | 标记为 `untested-newer`；轻量探测通过后允许使用，并向用户保留未验证提示 |
| Codex 未登录 | Runtime 可被发现，但 AI 任务不可进入可运行状态 |
| 手动路径无效 | 显示具体探测错误，并继续允许用户改回系统或标准安装路径 |

RocketX 当前桌面安装包采用 slim/full 分层。默认 slim 不捆绑 Codex 或 DSH，只负责探测系统安装；系统 DSH 只有在版本被 RocketX 验证为 `0.1.0-rc.6` 时才可用，更高版本需先升级 RocketX 支持线。Windows full 额外在私有资源目录中安装固定的 Codex 0.144.4、DSH 0.1.0-rc.6、私有 Node 与 OCR。macOS 和 Linux 官方包目前仍是 slim-only。

## 4. DSH Runtime 兼容结论

| 条件 | 当前表现 |
| --- | --- |
| 官方 slim 发现到可运行的系统 DSH | 可启动 DSH bridge，并打开官方 DSH Web 的本地 loopback 页面；AI 托管/共享 Agent 仍通过 controller/host 原生 RPC 交互 |
| Windows full 私有 `@deepseek-ai/dsh@0.1.0-rc.6` 运行时 | 可启动 DSH bridge，并打开官方 DSH Web 的本地 loopback 页面；AI 托管/共享 Agent 仍通过 controller/host 原生 RPC 交互 |
| DSH 不可用 | DSH 后端不可用并显示诊断；不影响 Codex 与确定性界面 |
| 当前 DSH provider 的凭据未配置或无效 | 可以查看 DSH 状态和配置；发送会 fail-closed，并展示 provider 返回的诊断与 DSH 配置入口 |
| 随包 DSH 资源缺失/不完整 | 拒绝启动该后端，不在运行时下载或猜测兼容版本；继续使用通过探测的 Codex，均不可用时以无 AI 启动 |
| DSH bridge 退出 | 清理旧审批/问题/队列，把运行 Session 标为中断；重连后从原生历史恢复 |

正式安装包采用 slim/full 分层：slim 只做系统探测，系统 DSH 必须先通过 RocketX 的 `0.1.0-rc.6` 验证；Windows full 另带私有 DSH 生产运行树和私有 Node。开发环境可通过 `sourcePath` 或 `ROCKETX_DSH_SOURCE` 显式覆盖源码构建；正式运行默认不依赖兄弟源码仓库。

## 5. 规格维护检查

改动下列任一事实时，必须同时更新本矩阵和对应能力域：

- 新增或删除导航入口；
- 改变 Web、桌面端、性能模式或无 Codex 场景的可用性；
- 改变数据落点、审批策略、调度存活条件或版本门禁；
- 改变 Codex/DSH 后端能力、固定 DSH 版本或系统 Node.js 要求；
- 把“未实现/部分实现”提升为“已实现”。
