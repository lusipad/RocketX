...
> 基线：工作树 `2026-08-16`
> 平台：共享 Agent 仅桌面端；继承当前启动级 Codex / DeepSeek / 无 AI 选择
...
1. 具备全局管理员、机器人账号、群主、群管理员或负责人角色的用户在房间或话题点击“AI 托管”，使用当前启动级 AI 运行时，选择当前托管项目，并确认本次会话的模型、推理、Agent（如适用）与权限。
...
- 只有 Rocket.Chat 服务端确认的全局管理员、机器人账号、群主、群管理员或负责人可以开启并发布权威托管租约；普通成员仍可在已有托管中使用 `@ai`。
- 可读状态正文和历史 marker 都属于不可信房间输入；只有发布者角色通过服务端复核、租约落在服务端消息时间的短期窗口内时，卡片才参与跨设备宿主仲裁。
...
- `AGT-AC-10`：普通成员伪造可读卡片或旧 marker 不得进入权威远端租约，也不得阻止已授权宿主开启、恢复或响应托管；超出消息时间窗口的租约同样拒绝。
