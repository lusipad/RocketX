...
- 任何能通过 Rocket.Chat 正常向当前房间发送消息的成员都可以开启并发布托管租约；客户端不再额外查询全局角色、房间角色或成员目录。
- 可读状态正文和历史 marker 都属于不可信房间输入；消息作者必须与卡片宿主一致，房间/话题必须匹配，且租约必须落在服务端消息时间的短期窗口内，卡片才参与跨设备宿主仲裁。
...
- `AGT-AC-10`：普通成员发布的有效托管卡片可以进入权威远端租约；消息作者与旧 marker 宿主不一致、房间/话题不匹配或超出消息时间窗口的租约必须拒绝。
