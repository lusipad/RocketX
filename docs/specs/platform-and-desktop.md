...
- 关闭到托盘、托盘恢复与真正退出；
- 操作系统开机启动设置；
- 签名 GitHub/HTTP 更新，以及可使用签名、或由本机探测固定 SHA-256 的受信任目录更新源；
- 原生文件对话框、保存、打开和定位；
- 本地图片 OCR 与精简/全量包差异；
- 标准/性能运行模式；
...
4. 管家等本地 Codex/DSH 执行能力显示真实不可用状态，不能调用 Tauri 命令；房间里由桌面宿主公开的有效共享托管状态仍可见，成员可继续使用 `@ai` 向该宿主提问。
...
4. 用户可在设置中读取/切换开机启动，由受控 Rust 命令调用系统插件登记带 `--autostart` 来源标记的登录项；正式安装版启动时刷新已启用项，本地 `debug` / `release` 构建不得读取或改写系统启动项。
...
11. DeepSeek 视图启动时优先探测系统里已安装且可运行、且已被 RocketX 验证为 `0.1.0-rc.6` 的 DSH；Windows full 则先从私有资源解析并验证固定 DSH CLI 与 bridge、私有 Node 和 OCR，并把持久数据写入应用数据目录；DSH 资源缺少或损坏时不下载替代版本、也不回退系统 Node，而是继续探测可用 Codex，均不可用时以无 AI 启动。
...
- `WebView2 正常/低内存`：可见并聚焦时为正常目标；隐藏、最小化或失焦时为低内存目标，后台连接和通知继续运行。
...
| 更新源不可达、远程签名失败或目录摘要变化 | 显示自定义源错误或安静跳过默认离线检查 | 不安装未通过对应来源完整性门禁的产物 |
...
- `PLAT-AC-06`：全量包与精简包的 OCR 差异明确；默认 slim 只探测系统安装的 Codex/DSH，不携带这两个运行时；Windows full 额外包含固定 Codex / DSH / Node / OCR 私有资源。
...
- `PLAT-AC-12`：正式安装包只让 slim 探测已安装运行时；系统 DSH 只有在 RocketX 验证为 `0.1.0-rc.6` 时才可用，Windows full 携带精确锁定且自证版本/默认 profile 的 DSH 生产运行树与私有 Node/Codex/OCR；运行时只接受随包资源或显式开发覆盖，不在线获取未知版本，也不借用系统 Node。
...
- Web 当前不能承载管家或共享 Agent 的本地执行；它可以投影桌面宿主公开的有效共享托管状态并向该宿主发送 `@ai`，但不是完整桌面替代。
