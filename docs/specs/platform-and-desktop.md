...
11. DeepSeek 视图启动时优先探测系统里已安装且可运行、且已被 RocketX 验证为 `0.1.0-rc.6` 的 DSH；Windows full 则从私有资源解析固定 DSH CLI 与 bridge、私有 Node 和 OCR，并把持久数据写入应用数据目录；缺少资源时明确失败，不在运行时下载替代版本，也不回退系统 Node。
