...
The current release target is `v0.42.7`. A `0.x` release must pass the version, changelog, trusted-tag, build, artifact, checksum, and explicit publication controls below, but it does not claim 1.0 maturity. npm publication is an independent package-delivery step and does not block a verified desktop/GitHub Release. Real product visuals and two external developer runs become mandatory only when the major version is 1 or higher.

`v0.42.7` is a patch release that releases RocketX's shared Codex app-server writer before launching an existing Butler thread in Codex Desktop. Controller-operation leases prevent reconnects, scheduled work, attachment preparation, thread creation, refresh, rename, or archive operations from racing the ownership transfer. The release line remains split: default slim desktop artifacts only probe already-installed Codex/DSH runtimes, and a system DSH is accepted only when RocketX has verified the exact `@deepseek-ai/dsh@0.1.0-rc.6` support line. The Windows full installer additionally ships the exactly locked `@deepseek-ai/dsh@0.1.0-rc.6` production runtime archive, a private Node payload, the fixed Codex payload, and OCR resources into private application data. A source checkout of `deepseek-harness` is never a release prerequisite. The `v0.42.0`, `v0.42.1`, and `v0.42.2` candidate tags were not published after their desktop matrices exposed missing pre-test preparation, raw DSH packaging limits, and the full-asset upload bug; `v0.42.4` was stopped before publication when Butler streaming flicker was found.
