...
The current release target is `v0.42.0`. A `0.x` release must pass the version, changelog, trusted-tag, build, artifact, checksum, and explicit publication controls below, but it does not claim 1.0 maturity. npm publication is an independent package-delivery step and does not block a verified desktop/GitHub Release. Real product visuals and two external developer runs become mandatory only when the major version is 1 or higher.

`v0.42.0` adds the DeepSeek Harness backend. Every desktop artifact must contain the exactly locked `@deepseek-ai/dsh@0.1.0-rc.6` production runtime tree and pass its build-time CLI/profile checks. Node.js itself is not bundled in this release; DSH users need system Node.js 22.19+ or 24+. A source checkout of `deepseek-harness` is never a release prerequisite.
