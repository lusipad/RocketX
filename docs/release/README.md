...
The current release target is `v0.43.5`. A `0.x` release must pass the version, changelog, trusted-tag, build, artifact, checksum, and explicit publication controls below, but it does not claim 1.0 maturity. npm publication is an independent package-delivery step and does not block a verified desktop/GitHub Release. Real product visuals and two external developer runs become mandatory only when the major version is 1 or higher.

`v0.43.5` builds on the verified `v0.43.4` transport and cross-platform packaging line. It removes the fixed DeepSeek-key precondition so DSH room conversations and shared hosting follow the provider selected in DSH; repairs private-session interruption and repeated-focus behavior; removes the blocking room-AI backdrop; lets conversational Markdown tables and code use the available width; and splits oversized shared-AI replies against Rocket.Chat's public `Message_MaxAllowedSize` while preserving Unicode, Markdown fences, indentation, ordering, and a single sender. Development builds also skip the production updater, while release builds keep the existing remote-greater-than-current comparison. The release line remains split: default slim desktop artifacts only probe already-installed Codex/DSH runtimes, and a system DSH is accepted only when RocketX has verified the exact `@deepseek-ai/dsh@0.1.0-rc.6` support line. The Windows full installer additionally ships the exactly locked `@deepseek-ai/dsh@0.1.0-rc.6` production runtime archive, a private Node payload, the fixed Codex payload, and OCR resources into private application data. Uninstalling either package may remove RocketX-owned private resources but must never remove npm/pnpm global DSH installations. A source checkout of `deepseek-harness` is never a release prerequisite. Earlier unpublished candidates remain documented in their changelog entries.
