...
`v0.43.2` is the patch release for the current runtime line. It keeps the startup-selected Codex / DSH / no-AI architecture from `v0.43.0`, but tightens the shared-hosting and message-rendering path: Web keeps shared hosting and `@ai` information architecture even when no local runtime is active; Rocket.Chat cross-client hosting cards stay plain readable text instead of hidden lease payloads while RocketX remains backward-compatible with the old marker format and accepts authority only from server-verified global or room hosts with a bounded lease; UI fixtures now explicitly choose the runtime they need instead of relying on a stale Web default; and the managed conversation / Markdown path no longer flickers during streaming. The release line remains split: default slim desktop artifacts only probe already-installed Codex/DSH runtimes, and a system DSH is accepted only when RocketX has verified the exact `@deepseek-ai/dsh@0.1.0-rc.6` support line. The Windows full installer additionally ships the exactly locked `@deepseek-ai/dsh@0.1.0-rc.6` production runtime archive, a private Node payload, the fixed Codex payload, and OCR resources into private application data. A source checkout of `deepseek-harness` is never a release prerequisite. The immutable `v0.43.0` candidate was not published after its main-line UI gate exposed the Web runtime drift; `v0.43.1` supersedes it. The `v0.42.0`, `v0.42.1`, and `v0.42.2` candidate tags were not published after their desktop matrices exposed missing pre-test preparation, raw DSH packaging limits, and the full-asset upload bug; `v0.42.4` was stopped before publication when Butler streaming flicker was found.
