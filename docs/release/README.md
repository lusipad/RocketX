...
The current release target is `v0.43.2`. A `0.x` release must pass the version, changelog, trusted-tag, build, artifact, checksum, and explicit publication controls below, but it does not claim 1.0 maturity. npm publication is an independent package-delivery step and does not block a verified desktop/GitHub Release. Real product visuals and two external developer runs become mandatory only when the major version is 1 or higher.

`v0.43.2` is the patch release for the current runtime line. It keeps the startup-selected Codex / DSH / no-AI architecture from `v0.43.0`, but tightens the shared-hosting and message-rendering path: Web keeps shared hosting and `@ai` information architecture even when no local runtime is active; any member who can speak in a room can start hosting through the RocketX control, while copied visible text and historical markers remain display-only and new cross-client arbitration accepts only the lease metadata or lease message id emitted by that control; bounded, role-verified `v0.43.1` claims remain readable during mixed-version upgrades; UI fixtures explicitly choose the runtime they need instead of relying on a stale Web default; and managed Markdown streams progressively in stable message nodes instead of flickering or being rebuilt at completion. The release line remains split: default slim desktop artifacts only probe already-installed Codex/DSH runtimes, and a system DSH is accepted only when RocketX has verified the exact `@deepseek-ai/dsh@0.1.0-rc.6` support line. The Windows full installer additionally ships the exactly locked `@deepseek-ai/dsh@0.1.0-rc.6` production runtime archive, a private Node payload, the fixed Codex payload, and OCR resources into private application data. A source checkout of `deepseek-harness` is never a release prerequisite. The immutable `v0.43.0` candidate was not published after its main-line UI gate exposed the Web runtime drift; the published `v0.43.1` release is superseded by this patch. The `v0.42.0`, `v0.42.1`, and `v0.42.2` candidate tags were not published after their desktop matrices exposed missing pre-test preparation, raw DSH packaging limits, and the full-asset upload bug; `v0.42.4` was stopped before publication when Butler streaming flicker was found.
...
4. `Desktop Build` creates a draft Release and verifies the Windows NSIS slim, MSI, and full installers; the macOS universal DMG and updater archive; the Linux AppImage, DEB, and RPM packages; updater metadata and signatures; and the plugin bundle. Slim artifacts only probe installed runtimes and accept system DSH only after RocketX has verified the exact `0.1.0-rc.6` support line; the Windows full installer carries the private DSH/Codex/private-Node payloads and the OCR resources. It writes release notes from `CHANGELOG.md` and uploads a directly usable `SHA256SUMS.txt`. It does not publish the draft.
