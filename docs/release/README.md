...
The current release target is `v0.43.4`. A `0.x` release must pass the version, changelog, trusted-tag, build, artifact, checksum, and explicit publication controls below, but it does not claim 1.0 maturity. npm publication is an independent package-delivery step and does not block a verified desktop/GitHub Release. Real product visuals and two external developer runs become mandatory only when the major version is 1 or higher.

`v0.43.4` carries the `v0.43.3` transport hardening patch and corrects its Unix DSH discovery build failure so the macOS universal and Linux release jobs compile again. It keeps the startup-selected Codex / DSH / no-AI architecture from `v0.43.0`: RocketX supports both HTTP and HTTPS server URLs, HTTPS uses the operating system trust chain instead of disabling certificate validation, the extra WebView Origin header is removed from desktop login requests, and staged login diagnostics surface redacted actionable detail for network, certificate, server-response, and credential failures. Slim runtime discovery recognizes npm/pnpm global DSH installations across store generations and gives an actionable install-or-Full message when no verified system DSH exists. Private room DeepSeek sessions use the projectless system workspace and can open native DSH configuration directly when credentials are missing; the shared-hosting start panel can add a local project through the system directory picker instead of redirecting to a separate project page. The release line remains split: default slim desktop artifacts only probe already-installed Codex/DSH runtimes, and a system DSH is accepted only when RocketX has verified the exact `@deepseek-ai/dsh@0.1.0-rc.6` support line. The Windows full installer additionally ships the exactly locked `@deepseek-ai/dsh@0.1.0-rc.6` production runtime archive, a private Node payload, the fixed Codex payload, and OCR resources into private application data. Uninstalling either package may remove RocketX-owned private resources but must never remove npm/pnpm global DSH installations. A source checkout of `deepseek-harness` is never a release prerequisite. The immutable `v0.43.3` candidate was not published after its release matrix exposed the Unix compile error; `v0.43.4` is the corrected patch. Earlier unpublished candidates remain documented in their changelog entries.
